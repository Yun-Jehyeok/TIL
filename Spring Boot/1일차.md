...
- [스프링부트생성](https://start.spring.io/)
- Thymeleaf 템플릿 엔진 사용을 위해서는 JAR 패키징 선택 필요
  - 타임리프 : View template으로 html 태그 기반의 동적 View 제공
- Dependency에 Lombok을 추가했는데 이는 반복되는 getter, setter, toString 등의 메서드 작성 코드를 줄여주는 코드 다이어트 라이브러리
...
위와 같이 @SpringBootApplication 어노테이션이 붙은 클래스가 메인 클래스입니다.

@SpringBootApplication으로 스프링 부트의 자동 설정, 스프링 Bean 읽기, 생성이 모두 자동 설정됩니다.

@SpringBootApplication 위치부터 설정을 읽기 때문에 이 클래스는 항ㅅ앙 프로젝트 최상단에 위치해야 합니다.

<br/>

# Hello 출력

1. src > main 밑에 controller 패키지를 만들고 안에 HelloController.class를 만든다.
...
- @RestController : REST API 개발에 사용되며 JSON 형태로 객체 데이터를 반환해줍니다.
- @GetMapping : HTTP > GET 사용
  <br/>

2. test 폴더에 controller 패키지를 만들고 HelloControllerTest.class 를 생성한다.
...
- @ExtendWith(SpringExtension.class) : 테스트 진행시 JUnit에 내장된 실행자 외에 다른 실행자를 실행시킨다.
- @WebMvcTest : Controller 레이어만 슬라이스 테스트 하고 싶을 때 주로 사용
- @Autowired : 스프링이 관리하는 빈을 주입 받음
- MockMvc : 웹 API 테스트시 사용
- status() : res.status(200) 이런 느낌
